...
def compute_sku(variant_of, attributes):
	...
	new_sku = _resolve_sku(variant_of, mock_doc, "(preview)")
	return {"sku": new_sku or variant_of}
...
def generate_variant_sku(doc, method=None):
	...
	new_sku = _resolve_sku(pattern, doc, doc.item_code or "(new)")
	if new_sku is None:
...
def _resolve_sku(pattern, variant_doc, item_label):
	"""
	Core SKU resolution: finds placeholders in pattern, builds the attr→sku_code
	map, validates all placeholders are satisfied, and returns the substituted SKU.

	Returns:
		str | None: The generated SKU string, or None if pattern has no placeholders.
	"""
	placeholders = _PLACEHOLDER_RE.findall(pattern)
	if not placeholders:
		return None

	attr_code_map = _build_attr_code_map(variant_doc)
	_validate_placeholders(placeholders, attr_code_map, item_label=item_label)
	return _PLACEHOLDER_RE.sub(lambda m: attr_code_map[m.group(1).lower()], pattern)
